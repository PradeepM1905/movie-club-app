...
# QUIZ MANAGEMENT
# ---------------------------------------
def has_user_taken_quiz(user_name, sprint_id):
    """Check if user has already taken the quiz for a sprint"""
    try:
        quiz_results = load_sheet("QuizResults")
        for result in quiz_results:
            if (result.get('Username') == user_name and 
                result.get('SprintId') == sprint_id):
                return True
        return False
    except Exception as e:
        st.warning(f"Error checking quiz results: {e}")
        return False

def get_quiz_questions(sprint_id):
    """Get quiz questions for a sprint"""
    try:
        quiz_questions = load_sheet("QuizQuestions")
        return [q for q in quiz_questions if q.get('SprintId') == sprint_id]
    except Exception as e:
        st.warning(f"Error loading quiz questions: {e}")
        return []

def generate_quiz_questions(sprint_id, movies_data):
    ...
    try:
        ...
        for movie in movies_data:
            movie_context += f"Movie: {movie.get('movie_name', '')}, Genre: {movie.get('genre', '')}, Description: {movie.get('description', '')}\n"
        
        prompt = f"""
        Create 12 trivia questions about these movies from a movie club sprint. 
        Create 2 questions for each of the 6 movies.
        
        Movies:
        {movie_context}
        
        Requirements:
        - 12 multiple-choice questions total (2 per movie)
        - 4 options for each question (A, B, C, D)
        - Mark the correct answer clearly
        - Questions should be about plot, characters, actors, director, release year, or interesting facts
...
def save_quiz_questions(sprint_id, questions):
    """Save generated questions to Google Sheets"""
    try:
        # Try to get existing worksheet, create if doesn't exist
        try:
            ws = sheet.worksheet("QuizQuestions")
        except:
            ws = sheet.add_worksheet(title="QuizQuestions", rows="1000", cols="10")
            # Add headers
            ws.append_row(["SprintId", "Question", "Option1", "Option2", "Option3", "Option4", "RightAnswer"])
        
        # Clear existing questions for this sprint
        existing_questions = ws.get_all_records()
        rows_to_delete = []
        
        for i, row in enumerate(existing_questions, start=2):
            if row.get('SprintId') == sprint_id:
        ...
        st.error(f"Error saving quiz questions: {e}")
        return False

def save_quiz_results(sprint_id, username, answers, total_score):
    """Save quiz results to Google Sheets"""
    try:
        # Try to get existing worksheet, create if doesn't exist
        try:
            ws = sheet.worksheet("QuizResults")
        except:
            ws = sheet.add_worksheet(title="QuizResults", rows="1000", cols="20")
            # Add headers
            headers = ["SprintId", "Username", "Timestamp"]
            headers.extend([f"Q{i+1}" for i in range(12)])
            headers.append("Total")
            ws.append_row(headers)
        
        # Prepare row data
        row_data = [sprint_id, username, str(get_current_datetime())]
...
# Add Quiz to menu - check if it's sprint start day
previous_sprint = get_previous_sprint()
current_sprint = get_current_sprint()
if previous_sprint and current_sprint:
    current_date = get_current_date()
    sprint_start = datetime.strptime(current_sprint['start_date'], '%Y-%m-%d').date()
    if current_date == sprint_start:
        menu.append("Movie Quiz")
...
if selected == "Dashboard":
    ...
    try:
        quiz_results = load_sheet("QuizResults")
        if quiz_results:
            # Calculate average quiz scores per user
            quiz_scores = {}
            for result in quiz_results:
                ...
                if isinstance(score, str):
                    try:
                        ...
                        score = 0
                if user not in quiz_scores:
                    quiz_scores[user] = []
                quiz_scores[user].append(score)
            
            # Create quiz leaderboard
            quiz_leaderboard = []
            for user, scores in quiz_scores.items():
                avg_score = sum(scores) / len(scores)
                quiz_leaderboard.append({
                    "User": user,
                    "Average Quiz Score": round(avg_score, 1),
                    "Quizzes Taken": len(scores)
                })
            
            quiz_leaderboard.sort(key=lambda x: x['Average Quiz Score'], reverse=True)
            df_quiz_leaderboard = pd.DataFrame(quiz_leaderboard)
            st.dataframe(df_quiz_leaderboard, use_container_width=True)
        else:
...
# ---------------------------------------
    ...
    # In Admin Panel section, add this after User Management
    st.subheader("Quiz Management")
    
    tab1, tab2 = st.tabs(["Generate Quiz", "Quiz Results"])
    
    with tab1:
        st.write("Generate quiz questions for a sprint")
        sprints = load_sheet("Sprints")
        sprint_options = [s['sprint_id'] for s in sprints]
        selected_sprint = st.selectbox("Select Sprint", sprint_options)
        
        if st.button("Generate Quiz Questions"):
            with st.spinner("Generating questions..."):
                movies = load_sheet("Suggestions")
                sprint_movies = [m for m in movies if m.get('sprint') == selected_sprint]
                
                if sprint_movies:
                    questions = generate_quiz_questions(selected_sprint, sprint_movies)
                    if questions and save_quiz_questions(selected_sprint, questions):
                        st.success(f"✅ Generated 12 questions for {selected_sprint}")
                    else:
                        st.error("Failed to generate questions")
                else:
                    st.error("No movies found for selected sprint")
    
    with tab2:
        st.write("View quiz results")
        try:
            quiz_results = load_sheet("QuizResults")
            if quiz_results:
...
# PAGE: MOVIE QUIZ
# ---------------------------------------
elif selected == "Movie Quiz":
    st.header("🎯 Movie Quiz")
    
    previous_sprint = get_previous_sprint()
    if not previous_sprint:
        st.warning("No previous sprint found for quiz.")
        st.stop()
    ...
    if has_user_taken_quiz(user_name, previous_sprint['sprint_id']):
        st.success("✅ You have already taken the quiz for this sprint!")
        st.info("You can only take the quiz once per sprint.")
        st.stop()
    
    # Check if questions exist, if not generate them
    questions = get_quiz_questions(previous_sprint['sprint_id'])
    ...
    if not questions:
        ...
        movies = load_sheet("Suggestions")
        previous_sprint_movies = [m for m in movies if m.get('sprint') == previous_sprint['sprint_id']]
        
        if not previous_sprint_movies:
            st.error("No movies found for the previous sprint.")
            st.stop()
        
        with st.spinner("Generating questions using AI..."):
            generated_questions = generate_quiz_questions(previous_sprint['sprint_id'], previous_sprint_movies)
        
        if generated_questions and len(generated_questions) == 12:
            if save_quiz_questions(previous_sprint['sprint_id'], generated_questions):
                questions = get_quiz_questions(previous_sprint['sprint_id'])
                st.success("✅ Quiz questions generated successfully!")
            else:
                st.error("Failed to save quiz questions.")
                st.stop()
        else:
    ...
    if 'user_answers' not in st.session_state:
        st.session_state.user_answers = [None] * 12
    ...
    if not st.session_state.quiz_started and not st.session_state.quiz_completed:
        st.write(f"**Quiz for Sprint: {previous_sprint['sprint_id']}**")
        st.write("You will have 20 seconds to answer each of the 12 questions.")
        st.write("Total time: 4 minutes")
    ...
    # Quiz in progress
        ...
        if current_q < len(questions):
            ...
            col1, col2 = st.columns([3, 1])
            with col1:
                st.subheader(f"Question {current_q + 1}/12")
            with col2:
            ...
            # Auto-advance or manual submit
            if time_remaining <= 0:
                st.session_state.user_answers[current_q] = selected_option if selected_option else "TIMEOUT"
            ...
            # Calculate score
            score = 0
            for i, (user_ans, question) in enumerate(zip(st.session_state.user_answers, questions)):
                if user_ans == question['RightAnswer']:
                    score += 1
            
            # Save results
            if save_quiz_results(previous_sprint['sprint_id'], user_name, st.session_state.user_answers, score):
                st.success("🎉 Quiz Completed!")
                st.balloons()
            else:
                st.error("Failed to save quiz results. Please contact admin.")
    ...
    if st.session_state.quiz_completed:
        ...
        for i, (user_ans, question) in enumerate(zip(st.session_state.user_answers, questions)):
            is_correct = user_ans == question['RightAnswer']
            if is_correct:
                score += 1
            
            results_data.append({
                "Question": i + 1,
                "Your Answer": user_ans if user_ans != "TIMEOUT" else "Timeout",
                "Correct Answer": question['RightAnswer'],
                "Status": "✅ Correct" if is_correct else "❌ Incorrect" if user_ans != "TIMEOUT" else "⏰ Timeout"
            })
        
        st.metric("Your Score", f"{score}/12")
